# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 08:42:16 2020

@author: Abdul Qayyum
"""
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% You can try these perfromnace metrics in your project or you can made your 
#################################################### own functions for perfromance evluation ###################
...
def draw_roc_curve(gt,pred,multi=False,num_class = 4,img_height = 256,img_width = 256):
    ...
    @param gt: ground truth mask(s) of shape [num_masks, image_width, image_height, num_classes] for multi mask
                                             [image_width, image_height, num_classes] for single mask
    ...
    for single image
        pred = model.predict(x_test[ix],verbose = 1)
        gt = y_test[ix]
        draw_roc_curve(gt,pred)
    """
    ...
    # plt.show()  
#===================================================================================================
def surfaceAreas(masks,segs):

    if (len(masks.shape)==3):

        MasksSurface = np.zeros((1,masks.shape[2]))
        SegsSurface  = np.zeros((1,masks.shape[2]))

        total_surface = masks.shape[0]*masks.shape[1]

        for idx in range(masks.shape[2]):

            MasksSurface[0,idx] = float((np.count_nonzero(masks[...,idx]))/total_surface)
            SegsSurface[0,idx]  = float((np.count_nonzero( segs[...,idx]))/total_surface)

        return MasksSurface, SegsSurface

    if (len(masks.shape)==4):

        MasksSurface = np.zeros((masks.shape[0],masks.shape[3]))
        SegsSurface  = np.zeros((masks.shape[0],masks.shape[3]))

        total_surface = masks.shape[1]*masks.shape[2]

        for idx in range(masks.shape[3]):
            for batch in range(masks.shape[0]):

                MasksSurface[batch,idx] = float((np.count_nonzero(masks[batch,:,:,idx]))/total_surface)
                SegsSurface[batch,idx]  = float((np.count_nonzero( segs[batch,:,:,idx]))/total_surface)

        return MasksSurface, SegsSurface
#=======================================================================================================
# Reference : https://stackoverflow.com/questions/16399279/bland-altman-plot-in-python
#=======================================================================================================
...
#
# your test folder
# inside the test folder, you have two folders, GT masks folder and predicted mask folder
#
